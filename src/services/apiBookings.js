...
export async function getBookings({ filter = null, page }) {
  let query = supabase
    .from("bookings")
    .select(
      "id, created_at, start_date, end_date, num_of_nights, num_of_guests, total_price, status, cabins(name), guests(full_name, email)",
      { count: "exact" }
    );
